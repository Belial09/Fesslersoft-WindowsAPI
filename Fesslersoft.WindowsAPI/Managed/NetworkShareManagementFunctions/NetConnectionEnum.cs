...
namespace Fesslersoft.WindowsAPI.Managed.NetworkShareManagementFunctions
{
    ...
    public sealed class NetConnectionEnum
    {
        ...
        /// <param name="shareNetName">Pointer to a string that specifies a share name or computer name for the connections of interest. If it is a share name, then all the connections made to that share name are listed. If it is a computer name (for example, it starts with two backslash characters), then NetConnectionEnum lists all connections made from that computer to the server specified. This string is Unicode if _WIN32_WINNT or FORCE_UNICODE is defined.</param>
